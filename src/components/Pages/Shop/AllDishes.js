...
const AllDishes = () => {
    ...
    useEffect(() => {
        fetch(`https://whispering-tor-70957.herokuapp.com/foods?limit=${limit}&pageNumber=${pageNumber}`)
            .then(res => res.json())
            .then(data => {
                setDishes(data.sort((a, b) => a.price - b.price))
            })
    ...
    //     if (sort === "featured") {
    //         setDishes((allDishes) =>
    //         allDishes.sort((a, b) => a.createdAt - b.createdAt))
    //     }
    //     else if (sort === "low") {
    //         setDishes((allDishes) =>
    ...
    //     else if (sort === "high") {
    //         setDishes((allDishes) =>
    //         allDishes.sort((a, b) => b.price - a.price))
    //     }
