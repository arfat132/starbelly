...
const Seafood = () => {
    ...
    useEffect(() => {
        fetch(`http://localhost:5000/foods`)
            .then(res => res.json())
            .then(data => setSeafood(data.filter(food => food.category.includes('seafood'))))
    }, [seafood, limit, pageNumber])
    return (
        <>
        <div>
                ...
                <div className="container px-14 pb-16 mx-auto">
                    {seafood?.length ?
                        <div className="flex flex-wrap -m-4 mb-6">
                            {
                                seafood.slice(0, 8).map(seafoodDishe => <Food
                                    key={seafoodDishe._id}
                                    selectedFood={seafoodDishe}
                                ></Food>)
                            }
                        </div>
                        :
