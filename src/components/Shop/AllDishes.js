...
const AllDishes = () => {
    ...
    useEffect(() => {
        fetch(`http://localhost:5000/foods?limit=${limit}&pageNumber=${pageNumber}`)
            .then(res => res.json())
    ...
    return (
        <>
            <div><p className='flex items-center ml-4 mt-[-46px] text-gray-500'>Paginate by
                    ...
                    <div className="container px-20 pb-16 mx-auto">
                        {allDishes?.length ?
                            <div className="flex flex-wrap -m-4 mb-6">
                                {
                                    allDishes.map(allDishe => <Food
                                        key={allDishe._id}
                                        selectedFood={allDishe}
                                    ></Food>)
                                }
                            </div>
                            :
                        ...
                        <span className='text-black bg-red-700'><svg class="w-5 h-5 " fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd"></path></svg></span>
                            {
